package game 
{
	 ...
	 */
	public class GameStateBrickLeft extends FlxState 
	{
		[Embed(source = '../assets/BrickBuilding2.png')] private var stoneBlock:Class;
		[Embed(source = '../assets/BrickBuildingBG.png')] private var fallingBlock:Class;
		[Embed(source = '../assets/Static.png')] private var Static:Class;
		[Embed(source = '../assets/FallingTile.png')] private var fallingTile:Class;
		[Embed(source = '../assets/DeBrick.png')] private var imgDebris:Class;
		[Embed(source = '../assets/skyBlue16.png')] private var VoidBG:Class;
		[Embed(source = '../assets/Cloud.png')] private var Cloud:Class;
		
		private var soundGen:SfxrSynth = new SfxrSynth();
		private var firstRun:Boolean = true;
		private var blocksGroup:FlxGroup;
		private var cloudsGroup:FlxGroup;
		private var orbsCont:FlxGroup;
		private var player:Player;
		private var point:Finger;
		private var blockoffset:int = 6000;
		...
		public function GameStateBrickLeft() 
		{
			super();
			...
			cloudsGroup = new FlxGroup();
			
			for (var i:int = 0; i < 100; i++) {
				if(Math.random() < .2){
					var spr:FlxSprite = new FlxSprite(30 * i, (Math.random() * 50) - 50, Cloud);
					cloudsGroup.add(spr, true);
				}
			}
			cloudsGroup.scrollFactor.x = cloudsGroup.scrollFactor.y = .3;
		...
		public override function update():void {
			...
			brixEmitter.x = player.x- 150;
			brixEmitter.y = player.y - 150;
			rockRain.x = player.x - 150;
			...
			if (Math.abs(blockoffset - player.x) < 200) {
				generateGround();
			}
		...
		private function generateGround():void {
			...
			if (blockoffset < 1000 && FlxU.random() * 100 < 100) {
				if(nexus == null){
			...
			for (var j:int = 0; j < blocksGroup.members.length; j++) {
				if (Math.abs(blocksGroup.members[j].x - player.x) > 300) {
